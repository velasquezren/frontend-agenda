...
@Component({
  ...
  template: `
    <main class="relative flex min-h-dvh items-center justify-center overflow-hidden bg-slate-50/80 px-4 py-12">
      <!-- Decorativos de fondo suaves (Minimalist Glow) -->
      <div class="pointer-events-none absolute -top-40 -left-40 size-96 rounded-full bg-marca-500/10 blur-3xl"></div>
      <div class="pointer-events-none absolute -bottom-40 -right-40 size-96 rounded-full bg-slate-900/5 blur-3xl"></div>

      <div class="relative w-full max-w-sm">
        <!-- Encabezado con Logo e Identidad -->
        <div class="mb-8 flex flex-col items-center text-center">
          <div class="mb-3.5 flex size-14 items-center justify-center rounded-2xl bg-white p-2.5 shadow-md ring-1 ring-slate-900/5 transition-transform hover:scale-105">
        ...
        <div class="rounded-2xl border border-slate-200/80 bg-white/95 p-7 shadow-xl shadow-slate-200/50 backdrop-blur-xl">
          <form class="space-y-4" (submit)="entrar($event)">
            <!-- Campo Usuario -->
            <app-field etiqueta="Usuario" para="usuario">
              <div class="relative flex items-center">
                <span class="pointer-events-none absolute left-3 z-10 text-slate-400">
            ...
            <app-field etiqueta="Contraseña" para="password">
              <div class="relative flex items-center">
                ...
                <button
                  ...
                  class="absolute right-3 z-10 text-slate-400 hover:text-slate-600 focus:outline-none"
            ...
            @if (error()) {
              <div
                class="flex items-center gap-2.5 rounded-xl border border-red-200/80 bg-red-50/90 px-3.5 py-2.5 text-xs font-medium text-red-800 backdrop-blur-sm"
                role="alert"
              >
            ...
            <button
              ...
              appBtn
              tamano="lg"
              class="w-full justify-center rounded-xl shadow-md transition-all active:scale-[0.99] font-semibold"
              [disabled]="cargando()"
            >
              @if (cargando()) {
                <app-spinner />
        ...
        <!-- Pie de página / Conexión segura -->
        <div class="mt-8 flex items-center justify-center gap-1.5 text-xs text-slate-400">
